...
import { API_ROUTES, type PLATFORMS } from './Enum';
import type {
	IndexItem,
...
valid values: true, false
   ...
   */
	 ...
	 */
	public async coinIdHistory(input: { id: string; date: string; localization?: boolean }) {
	 ...
	 *```
	 */
	public async coinIdOHLC(input: { id: string; vs_currency: string; days: number | 'max' }) {
	 ...
	 * @returns {CoinFullInfo}
	 */
	public async contract(input: { id: PLATFORMS; contract_address: string }) {
	 ...
	 * @returns {CoinMarketChartResponse} Get historical market data include price, market cap, and 24h volume
	 */
	public async exchanges(input: { per_page?: number; page?: number }) {
	 ...
	 * @returns {CoinStatusUpdateResponse} Get status updates for a given exchange
	 */
	public async exchangeIdVolumeChart(input: { id: string; days: number }) {
	 ...
	 * @returns {Finance[]}
	 */
	public async financePlatforms(input?: { per_page?: number; page?: number }) {
	 ...
	 * @returns {IndexItem[]}
	 */
	public async indexes(input?: { per_page?: number; page?: number }) {
	 ...
	 * @returns {IndexItem[]}
	 */
	public async indexesMarketId(input?: { market_id?: number; path_id?: number; id?: number }) {
	 ...
	 * @returns {Derivative[]}
	 */
	public async derivatives(input: { include_tickers?: 'all' | 'unexpired' }) {
